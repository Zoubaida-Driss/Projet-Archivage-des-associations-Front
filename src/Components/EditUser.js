...
const EditUser = () => {
    ...
    const handleSubmit=()=>{
      ...
      const Edituser = (info,id) => {
        AssociationService.Edit_user(info,id)
          .then((response) => {
            console.log(response);
            setNomuser(response.data.nom_user)
            setPrenomuser(response.data.prenom_user)
            setEmail(response.data.email)
            setMotdepasse(response.data.motdepasse)
            setIdrole(response.data.id_role)
      ...
      <div className="col-12">
        <div className="card">
          <div className="card-header">
            <h5 className="title">Modifier un utilisateur</h5>
          </div>
